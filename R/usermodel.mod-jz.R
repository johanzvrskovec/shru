...
usermodel.mod <-function(covstruc,estimation="DWLS", model = "", CFIcalc=TRUE, std.lv=FALSE, imp_cov=FALSE,fix_resid=TRUE,toler=FALSE, parseResults=FALSE, generateDOT=FALSE){ 
  time<-proc.time()
  ...
  #mod- initialise new return objects
  primaryLavaanModelFittingResults<-NA
  resid_cov<-NA
  results.parsed<-NA
  path.graph.dot<-NA
  ...
  if(toler == FALSE){
    ...
    if(class(bread2$value)[1] == "matrix"){
      ...
      #mod - storing the model1 results from here
      primaryLavaanModelFittingResults<-Model1_Results
    }
  ...
  if(class(bread2$value)[1] == "matrix" & check == 2){  
    ...
    #mod - placed a general imp_cov computation here rather than in a completely separate return block.
    if(imp_cov == TRUE){
      resid_cov<-list()
    ...
    #mod - type fixes and text parsing of results for dot-models
    if(!is.null(results)){
    ...
    #mod - only one return statement
    return(list(modelfit=modelfit,results=results,lresults=primaryLavaanModelFittingResults,resid_cov=resid_cov, results.parsed=results.parsed, path.graph.dot=path.graph.dot))
